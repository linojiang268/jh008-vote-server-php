(function(window, document) {
    ...
    Table.prototype.setStatus = function(statusOptionName, value) {
        var statusOptions = this.statusOptions,
            flag = true;
        if (statusOptionName == 'refresh') {
            $.each(statusOptions.refresh, function(index, ref) {
                if (ref.key == value.key) {
                    statusOptions.refresh.splice(index, 1, value);
                    flag = false;
                }
            })
            if (flag) 
                this.statusOptions.refresh.push(value);
        } else if (statusOptionName == 'orderBy') {
            $.each(statusOptions.orderBy, function(index, orderby) {
                if (orderby.key == value.key) {
                    statusOptions.orderBy.splice(index, 1, value);
                    flag = false;
                }
            })
            if (flag)
                this.statusOptions.orderBy.push(value);
        }
    }

    Table.prototype.clearStatus = function(name) {
        if (name && this.statusOptions[name]) {
            this.statusOptions[name] = [];
        } else {
            this.initStatus();
        }
    }
    ...
    Table.prototype.initMultiSelect = function() {
        ...
        if (this.options.multiselect) {
            ...
            this.options.events['click input.rowSelect'] = 'rowSelect';
    ...
    Table.prototype.getSelectAll = function() {
        var result = [];
        $.each(this.rowsList, function(index, row) {
            if (row.state.select) {
