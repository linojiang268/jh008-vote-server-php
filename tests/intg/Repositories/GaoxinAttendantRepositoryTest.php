...
use Jihe\Repositories\GaoxinAttendantRepository as AttendantRepository;
use \PHPUnit_Framework_Assert as Assert;
use Illuminate\Foundation\Testing\DatabaseTransactions;
use Illuminate\Support\Facades\Cache;

class GaoxinAttendantRepositoryTest extends TestCase
{
    ...
    {
        $this->createData(10000, '13800010000', AttendantRepository::STATUS_PENDING);
        $this->createData(10001, '13800010001', AttendantRepository::STATUS_PENDING);
        $this->createData(10002, '13800010002', AttendantRepository::STATUS_PENDING);

        Assert::assertTrue($this->getRepository()->removeApplicants([10000, 10001]));

        $this->notSeeInDatabase(AttendantRepository::TABLE, [
            'id'     => 10000,
            'status' => 0,
        ]);
        $this->notSeeInDatabase(AttendantRepository::TABLE, [
    ...
    {
        $this->createData(10003, '13800010003', AttendantRepository::STATUS_APPROVED);
        $this->createData(10004, '13800010004', AttendantRepository::STATUS_APPROVED);
        $this->createData(10005, '13800010005', AttendantRepository::STATUS_APPROVED);

        Assert::assertTrue($this->getRepository()->removeAttendants([10003, 10004]));
        ...
        $expectedId, $expectedMobile, $expectedStatus, $attendant)
    {
        ...
        Assert::assertEquals(0, $attendant->love_bus_apply);
    ...
    private static function createCacheKey($page, $key = AttendantRepository::ATTENDANT_LIST_KEY) {
        return md5($key) . '_' . date('Y-m-d') . '_' . $page;
    }

    /**
     * @return \Jihe\Repositories\GaoxinAttendantRepository
     */
    private function getRepository()
    {
        return $this->app[\Jihe\Repositories\GaoxinAttendantRepository::class];
    }
}
