...
class ChuanglanSmsServiceSpec extends ObjectBehavior
{
    function let(ClientInterface $client)
    {
        $this->beAnInstanceOf(\Jihe\Services\Sms\ChuanglanSmsService::class, [
                'account',    // account
                'password',   // password
                '123',       // affix
                $client
        ...
        $client->request('POST', 'http://222.73.117.158/msg/HttpBatchSendSM',
            Argument::that(function (array $request) {
                ...
                return $request['account']    == 'account' &&
                       ...
                       $request['extno'] == '123';
            }))->willReturn(new Response(200, [], file_get_contents(__DIR__ . '/test-data/Chuanglan/single_success.txt')));
        ...
        $client->request('POST', 'http://222.73.117.158/msg/HttpBatchSendSM',
            Argument::that(function (array $request) {
                ...
                $request['extno'] == '123';
                                              ...
                                              file_get_contents(__DIR__ . '/test-data/Chuanglan/multi_success.txt')));
    ...
    function it_should_have_quota_queried(ClientInterface $client)
    {
        $client->request('POST', 'http://222.73.117.158/msg/QueryBalance',
                         Argument::that(function (array $request) {
                             ...
                             return $request['account'] == 'account' &&
                             $request['pswd'] == 'password';
                         }))->willReturn(new Response(200, [], file_get_contents(__DIR__ . '/test-data/Chuanglan/surplus.txt')));
...
//               ->willReturn(new Response(200, [], file_get_contents(__DIR__ . '/test-data/Chuanglan/surplus.xml')));
//
