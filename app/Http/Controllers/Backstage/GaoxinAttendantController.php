...
class GaoxinAttendantController extends Controller
{
    ...
    public function attendant(Request $request, Guard $auth, WechatService $wechatService)
    {
        ...
        return view('backstage.wap.gaoxin.index', $data);
    }
    ...
    private function morphAttendant($stdAttendant)
    {
        return [
            ...
            'love_bus_apply' => $stdAttendant->love_bus_apply,
    ...
    public function removeAttendants(Request $request, AttendantRepository $attendantRepository)
    {
        ...
        try {
            ...
            if ($attendantRepository->removeAttendants($request->input('attendant'))) {
                return $this->json('删除成功');
            }
            return $this->jsonException('删除失败');
        } catch (\Exception $ex) {
    ...
    public function enroll(Request $request, AttendantRepository $attendantRepository, StorageService $storageService)
    {
        $this->validate($request, [
            ...
            'love_bus_apply.boolean' => '是否申请乘坐爱情大巴错误',
            'motto.max'           => '个人宣言错误',
        ...
        try {
            ...
            $attendant = [
                ...
                'love_bus_apply'=> $request->input('love_bus_apply', false),
    ...
    {
        $key = md5('gaoxin_attendant' . '_' . $attendant);
        if (Cache::has($key)) {
