...
class PlaneModelAttendantRepository
{
    const STATUS_PENDING = 0;
    const STATUS_APPROVED = 1;

    const ATTENDANT_LIST_KEY = 'plane_model_attendant_list';
    ...
    public function add(array $attendant)
    {
        ...
        array_set($attendant, 'gender', array_get($attendant, 'gender', 2));
        ...
        return \DB::insert('insert into plane_model_attendants (
                    name, gender, height, weight, bwh, shoe_size,
    ...
    {
        $rst = \DB::select('select id from plane_model_attendants where mobile = :mobile limit 1', ['mobile' => $mobile]) ?: null;
        if (empty($rst)) {
            return null;
        }
    ...
    public function approve($attendants)
    {
        ...
        $rst = count($attendants) === \DB::update(
            'update plane_model_attendants set status = 1 where id in (' .
    ...
    public function remove($attendants)
    {
        ...
        return count($attendants) === \DB::delete(
            'delete from plane_model_attendants where id in (' .
            implode(array_fill(0, count($attendants), '?'), ',') . ')',
            $attendants
        );
    }
    ...
    {
        $rst = \DB::select('select * from plane_model_attendants where id = ?', [$attendant]);
        if (empty($rst)) {
            return null;
        }
    ...
    public function listApprovedAttendants($page, $size, $cache = false)
    {
        ...
        $countRst = \DB::select('select count(1) AS total from plane_model_attendants where status=' . self::STATUS_APPROVED);
        $attendants = \DB::select('select * from plane_model_attendants where status=? order by id asc limit ?,?',
                                  [
    ...
    {
        $countRst = \DB::select('select count(1) AS total from plane_model_attendants where status=' . self::STATUS_PENDING);
        $attendants = \DB::select('select * from plane_model_attendants where status=' . self::STATUS_PENDING . ' limit ?,?', [($page - 1) * $size, $size]);
    ...
    {
        $countRst = \DB::select('select count(1) AS total from plane_model_attendants where status=? and id<?', [self::STATUS_APPROVED, $attendant->id]);
